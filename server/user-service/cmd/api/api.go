...
func (s *APIServer) Run() error {
	// Создания router и префикс /microservice/user-service
	router := mux.NewRouter()
	subrouter := router.PathPrefix("/microservice/user-service").Subrouter()
	...
	subrouter.HandleFunc("/adm/doc", func(w http.ResponseWriter, r *http.Request) {
		htmlContent, err := scalar.ApiReferenceHTML(&scalar.Options{
			SpecURL: "http://localhost:8001/microservice/user-service/docs",
