...
object Logger {
    ...
    def logServer(clientIP: String, method: String, url: String, status: String, message: String): Unit = {
        val timestamp = dateFormat.format(new Date())
        val logMessage = f"[$timestamp%-20s] Device: $clientIP%-15s | Method: $method%-5s | URL: $url%-30s | Status: $status%-5s | Message: $message%-50s"
